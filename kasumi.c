...
main ( int argc, char **argv ) {
     ...
     u8* clave=(u8 *)atol(argv[1]);
     u8* claro=(u8 *)atol(argv[2]);
  
     

     printf("El texto en claro es: %u ",(unsigned int)claro);
     
     KeySchedule(clave);
     Kasumi(claro);
     
     printf("El texto cifrado es %lu + %hu + %u ",(long unsigned int)d->b32,(int)d->b16,(unsigned int)d->b8);
...
void KeySchedule( u8 *k )
{
    ...
    printf("La clave usada es: %hd" ,key[n]);
